...
class Renderer {
    ...
    func foundPeople(_ texture: MTLTexture) -> Bool {
    ...
    func update() {
        ...
        if let commandBuffer = commandQueue.makeCommandBuffer() {
            ...
            if let renderPassDescriptor = renderDestination.currentRenderPassDescriptor, let currentDrawable = renderDestination.currentDrawable {
                ...
                sceneRenderDescriptor.colorAttachments[0].clearColor = MTLClearColor(red: Double(color!.redValue), green: Double(color!.greenValue), blue: Double(color!.blueValue), alpha: 0.5)
                ...
                if let sceneRenderEncoder = commandBuffer.makeRenderCommandEncoder(descriptor: sceneRenderDescriptor) {
                    ...
                    //if let toCheck = alphaTexture {
                    //if(!foundPeople(toCheck)){
                    //session
                ...
                if let compositeRenderEncoder = commandBuffer.makeRenderCommandEncoder(descriptor: renderPassDescriptor) {
                    ...
                    //compositeRenderEncoder.setBlendColor(red: 255, green: 0, blue: 0, alpha: 255)
